import 'package:flutter/material.dart';

class AppColors {
  static const Color primaryGreen = Color(0xFF1B5E20);
  static const Color primaryGreenLight = Color(0xFF2E7D32);
  static const Color primaryGreenDark = Color(0xFF0A3D0A);
  static const Color brandGreen = Color(0xFF4CAF50);
  static const Color accentGold = Color(0xFFFFC107);
  static const Color accentGoldLight = Color(0xFFFFD54F);
  static const Color accentGoldDark = Color(0xFFFFA000);

  // Splash
  static const Color splashBackground = Color(0xFFFFFFFF);
  static const Color splashIcon = Color(0xFF1A1A1A);

  static const Color backgroundLight = Color(0xFFFFFFFF);
  static const Color backgroundDark = Color(0xFF121212);
  static const Color surfaceLight = Color(0xFFFFFFFF);
  static const Color surfaceDark = Color(0xFF1E1E1E);
  static const Color cardLight = Color(0xFFFFFFFF);
  static const Color cardDark = Color(0xFF2C2C2C);

  // Map colors
  static const Color mapBackground = Color(0xFFE8EBE4);
  static const Color mapPark = Color(0xFFC8E6C9);

  // Drawer
  static const Color drawerBackground = Color(0xFF1C1C1C);

  // Quick action buttons
  static const Color quickActionBg = Color(0xFFF2F2F2);

  static const Color textPrimaryLight = Color(0xFF1A1A1A);
  static const Color textPrimaryDark = Color(0xFFF5F5F5);
  static const Color textSecondaryLight = Color(0xFF757575);
  static const Color textSecondaryDark = Color(0xFFAAAAAA);

  static const Color success = Color(0xFF4CAF50);
  static const Color error = Color(0xFFE53935);
  static const Color warning = Color(0xFFFF9800);
  static const Color info = Color(0xFF2196F3);

  static const Color dividerLight = Color(0xFFE0E0E0);
  static const Color dividerDark = Color(0xFF3A3A3A);

  static const Color googleBlue = Color(0xFF4285F4);
  static const Color googleRed = Color(0xFFDB4437);
  static const Color googleYellow = Color(0xFFF4B400);
  static const Color googleGreen = Color(0xFF0F9D58);
}
