...
/// Dedicated "Add Driver" screen, reached from the Drivers list.
/// Uses the same createDriver() call the old dialog used — no backend
/// change, just a fuller-page presentation of the same form.
...
class _CreateDriverScreenState extends State<CreateDriverScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (name.isEmpty) {
      setState(() => _error = 'Please enter the driver\'s name');
      return;
    }
    if (email.isEmpty) {
    ...
    try {
      final result = await _shuttleService.createDriver(
        name: name,
        email: email,
        password: password,
      ...
      if (result.success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Driver created: $email')),
        );
      } else {
        setState(() => _error = result.message ?? 'Failed to create driver');
      }
    } catch (e) {
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                'The driver signs in with this email and password, then you can assign them a shuttle from the Drivers list.',
              ...
              const SizedBox(height: 8),
              _FormField(controller: _emailController, hint: 'driver@example.com', keyboardType: TextInputType.emailAddress),
              const SizedBox(height: 16),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _isLoading
                      ...
                      : Text('Create Driver', style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.w600)),
                ),
              ),
            ],
          ),
